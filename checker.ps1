...
Get-ChildItem $folder -File -Recurse | ForEach-Object {
    ...
    if ($videoCodec -ne "hevc" -or $audioCodec -ne "aac") {
        ...
        Write-Host "$($_.FullName)" -BackgroundColor Yellow
        Write-Host "  Size: ${sizeMB} MB" -BackgroundColor Yellow
        Write-Host "  Video: $videoCodec" -BackgroundColor Yellow
        Write-Host "  Audio: $audioCodec" -BackgroundColor Yellow
        Write-Host ""
    }
}